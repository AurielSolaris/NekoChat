...
Qwen3Model::Qwen3Model(const std::string& dir, BackendPref pref, int threads, const ProgressFn& progress) {
    ...
    for (auto& kv : ts.all())
        if (kv.first != "lm_head.weight" || head_ == "lm_head.weight") params_ += kv.second.numel();
    ...
    progress(1.0f, "Ready");
    NEKO_LOGI("Qwen3 loaded: %lld params, %d layers, ctx %d, backend %s", (long long)params_, cfg_.nLayer,
...
bool Qwen3Model::gpuCompatible(const gpu::ComputeBackend& be, std::string& why) const {
    ...
    size_t biggest = std::max({size_t(2 * F) * size_t(C) * 2, size_t(qkvWidth()) * size_t(C) * 2,
                               size_t(kGpuChunkRows) * size_t(C) * 2, ctx * size_t(kvWidth()) * 2,
                               size_t(kMaxBatch) * size_t(2 * F) * 4, ctx * size_t(cfg_.headDim) * 4});
...
std::vector<uint16_t> Qwen3Model::rowsF16(TensorStore& ts, std::initializer_list<std::string> names) const {
    std::vector<uint16_t> out;
    for (auto& n : names) {
        auto v = ts.toF16(n, false);
...
void Qwen3Model::loadCpu(TensorStore& ts, const ProgressFn& progress) {
    ...
    embed_ = ts.toF16(prefix_ + "embed_tokens.weight", false);
    if (head_ == "lm_head.weight") lmHead_ = ts.toF16(head_, false);
    ...
    for (int l = 0; l < cfg_.nLayer; l++) {
        ...
        L.wqkv = rowsF16(ts, {layerName(l, "self_attn.q_proj.weight"), layerName(l, "self_attn.k_proj.weight"),
        ...
        L.wo = ts.toF16(layerName(l, "self_attn.o_proj.weight"), false);
        L.wgu = rowsF16(ts, {layerName(l, "mlp.gate_proj.weight"), layerName(l, "mlp.up_proj.weight")});
        L.wdown = ts.toF16(layerName(l, "mlp.down_proj.weight"), false);
...
void Qwen3Model::loadGpu(TensorStore& ts, const ProgressFn& progress) {
    ...
    auto f32 = [&](const std::vector<float>& v) { return be.create(v.size() * 4, v.data()); };
    auto f16 = [&](const std::vector<uint16_t>& v) { return be.create(v.size() * 2, v.data()); };
    auto chunked = [&](const std::string& name) {
        std::vector<uint16_t> all = ts.toF16(name, false);
        std::vector<Chunk> out;
        for (int r0 = 0; r0 < cfg_.nVocab; r0 += kGpuChunkRows) {
            int rows = std::min(kGpuChunkRows, cfg_.nVocab - r0);
            out.push_back({be.create(size_t(rows) * size_t(C) * 2, all.data() + size_t(r0) * size_t(C)), r0, rows});
        }
        return out;
    };
    ...
    gRope_ = f32(ropeTable(shape_.context));
    ...
    for (int l = 0; l < cfg_.nLayer; l++) {
        ...
        L.qkNorm = f32(qk);
        L.wqkv = f16(rowsF16(ts, {layerName(l, "self_attn.q_proj.weight"), layerName(l, "self_attn.k_proj.weight"),
                                  layerName(l, "self_attn.v_proj.weight")}));
        L.bqkv = nullptr;
        if (cfg_.qkvBias) {
            std::vector<float> b;
        ...
        L.wo = f16(ts.toF16(layerName(l, "self_attn.o_proj.weight"), false));
        L.wgu = f16(rowsF16(ts, {layerName(l, "mlp.gate_proj.weight"), layerName(l, "mlp.up_proj.weight")}));
        L.wdown = f16(ts.toF16(layerName(l, "mlp.down_proj.weight"), false));
        L.kCache = be.create(cacheBytes);
        L.vCache = be.create(cacheBytes);
    }
    const size_t B = kMaxBatch;
    gTokens_ = be.create(B * 4);
    gX_ = be.create(B * size_t(C) * 4);
    gXn_ = be.create(B * size_t(C) * 4);
    gQkv_ = be.create(B * size_t(qkvWidth()) * 4);
    gAtt_ = be.create(B * size_t(qWidth()) * 4);
    gH_ = be.create(B * size_t(2 * F) * 4);
    gG_ = be.create(B * size_t(F) * 4);
...
void Qwen3Model::forwardCpu(const int* tokens, int T, int pos0, float* logits) {
    ...
    for (int t = 0; t < T; t++) {
        const uint16_t* e = embed_.data() + size_t(tokens[t]) * size_t(C);
        float* xr = x_.data() + size_t(t) * size_t(C);
        for (int c = 0; c < C; c++) xr[c] = f16_to_f32(e[c]);
    }
    ...
    for (size_t l = 0; l < layers_.size(); l++) {
        ...
        cpu::matmulF16(pool, xn_.data(), T, C, L.wqkv.data(), L.bqkv.empty() ? nullptr : L.bqkv.data(), qkv_.data(),
                       QW, 0);
        ...
        cpu::matmulF16(pool, att_.data(), T, qWidth(), L.wo.data(), nullptr, x_.data(), C, cpu::kAccumulate);
        if (stopAt(l, 5, x_, C)) return;
        cpu::rmsNorm(x_.data(), xn_.data(), L.ln2.data(), T, C, cfg_.eps);
        if (stopAt(l, 6, xn_, C)) return;
        cpu::matmulF16(pool, xn_.data(), T, C, L.wgu.data(), nullptr, h_.data(), 2 * F, 0);
        if (stopAt(l, 7, h_, 2 * F)) return;
        cpu::siluMul(h_.data(), g_.data(), T, F);
        if (stopAt(l, 8, g_, F)) return;
        cpu::matmulF16(pool, g_.data(), T, F, L.wdown.data(), nullptr, x_.data(), C, cpu::kAccumulate);
    ...
    const uint16_t* head = lmHead_.empty() ? embed_.data() : lmHead_.data();
    cpu::matmulF16(pool, xn_.data(), 1, C, head, nullptr, logits, cfg_.nVocab, 0);
}
...
void Qwen3Model::forwardGpu(const int* tokens, int T, int pos0, float* logits) {
    ...
    for (const Chunk& ch : gEmbed_)
        run(Kernel::Embed, gTokens_, ch.buf, nullptr, gX_, {T, C, 0, 0, ch.row0, ch.rows}, uint32_t((C + 63) / 64),
            uint32_t(T));
    for (size_t l = 0; l < glayers_.size(); l++) {
        ...
        run(Kernel::Matmul, gXn_, L.wqkv, L.bqkv, gQkv_, {C, QW, T, L.bqkv ? 4 : 0}, uint32_t(QW), tTiles);
        ...
        run(Kernel::Matmul, gAtt_, L.wo, nullptr, gX_, {QH, C, T, 2}, uint32_t(C), tTiles);
        if (stopAt(l, 5, gX_, C)) return;
        run(Kernel::LayerNorm, gX_, gXn_, L.ln2, nullptr, {C, eps, 0, 1}, uint32_t(T), 1);
        if (stopAt(l, 6, gXn_, C)) return;
        run(Kernel::Matmul, gXn_, L.wgu, nullptr, gH_, {C, 2 * F, T, 0}, uint32_t(2 * F), tTiles);
        ...
        run(Kernel::Matmul, gG_, L.wdown, nullptr, gX_, {F, C, T, 2}, uint32_t(C), tTiles);
    ...
    for (const Chunk& ch : gHead_)
        run(Kernel::Matmul, gXn_, ch.buf, nullptr, gLogits_, {C, cfg_.nVocab, 1, 0, 0, ch.row0}, uint32_t(ch.rows), 1);
    be.submitAndWait();
